...
export function EditProductDialog({
  ...
  // Store existing images (with id) and new images (File objects)
  type ImageItem = 
    | { type: 'existing'; id: string; url: string; displayOrder: number }
    | { type: 'new'; file: File; preview: string; displayOrder: number }
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        // Convert all images (existing and new) to ProductGenericRequest format
        // Maintain the order as they appear in the images array
        const imageRequests = await Promise.all(
          images.map(async (img) => {
            if (img.type === 'existing') {
              // For existing images, send only the ID
              return {
                existingImage: img.id,
                newImage: null,
              }
            } else {
              // For new images, convert to base64 and send the image data
              ...
              return {
                ...
                newImage: {
                  ...
                  contentType: getContentType(img.file),
                },
              }
            }
          })
        ...
        // Update the product (cache will be updated automatically via onSuccess)
        await updateProductMutation.mutateAsync({
          ...
          data: {
            ...
            images: imageRequests.length > 0 ? imageRequests : undefined,
          },
        })
        
        // Only close dialog after successful API response
        onOpenChange(false)
  ...
  useEffect(() => {
    if (product) {
      ...
      // Convert images - sort by displayOrder
      if (product.images && product.images.length > 0) {
        const sortedImages = [...product.images].sort((a, b) => a.displayOrder - b.displayOrder)
        const convertedImages: ImageItem[] = sortedImages.map(img => ({
          type: 'existing',
          id: img.id,
          url: img.url,
  ...
  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.error('Erro ao validar imagens. Por favor, selecione apenas arquivos de imagem válidos.')
    }
              ...
              {images.length === 0 ? (
                ...
                <div className="flex gap-3 items-start">
                  ...
                  <div className="w-[60%] flex-shrink-0 relative aspect-square rounded-lg border-2 border-primary overflow-hidden bg-muted">
                    <img
                      src={images[0].type === 'existing' ? images[0].url : images[0].preview}
